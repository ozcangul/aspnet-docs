...
The ``Entries`` collection will have just one ``EntityEntry``, object, and that object has the new values entered by the user. You can get the current database values by using a `no-tracking query <https://ef.readthedocs.io/en/latest/querying/tracking.html>`__.
