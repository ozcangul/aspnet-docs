...
You'll create web pages that work with the Department entity and handle concurrency errors. The following illustrations show the Index and Delete pages, including some messages that are displayed if a concurrency conflict occurs.

.. image:: concurrency/_static/department-index.png
   :alt: Department Index page
...
.. image:: concurrency/_static/scaffold-department.png
...
This object has the new values entered by the user in its Entity property, and you can get the current database values by using a `no-tracking query <https://ef.readthedocs.io/en/latest/querying/tracking.html>`__.
...
Next, the code adds a custom error message for each column that has database values different from what the user entered on the Edit page (only one field is shown here).
...
.. image:: concurrency/_static/index-after-change-2.png
   :alt: Departments Index page after second edit
...
Update Index. Details, and Create views
---------------------------------------

The scaffolding engine created RowVersion columns in the Index and Details views.  What you want in those views is to show the Administrator, not the RowVersion column.

Change RowVersion to Administrator in *Views/Departments/Index.cshtml*, and change the heading to "Departments".

.. literalinclude::  intro/samples/cu/Views/Departments/Index.cshtml
  :language: html
  :emphasize-lines: 4,7,22,40

Change RowVersion to Administrator in *Views/Departments/Details.cshtml*, and change the heading to "Departments".

.. literalinclude::  intro/samples/cu/Views/Departments/Details.cshtml
  :language: html
  :emphasize-lines: 25-30

Add a Select option to the drop-down list in *Views/Create/Details.cshtml*.
...
This completes the introduction to handling concurrency conflicts. For information about other ways to handle various concurrency scenarios, see `Concurrency conflicts <https://ef.readthedocs.io/en/latest/saving/concurrency.html>`__. The next tutorial shows how to implement table-per-hierarchy inheritance for the Instructor and Student entities.